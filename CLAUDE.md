...
## 📐 교육과정 범위 제한 (현 고2 학생 대상)

사용자 요청 (2026-05-06): **현 고2 학생들이 치를 수능에서 미적분 과목이 빠짐**.

**자료 제작 시 반드시 제외할 미적분 과목 내용** (수학Ⅰ·수학Ⅱ 만 사용):
- ❌ **삼각함수의 덧셈정리** ($\sin(\alpha\pm\beta),\,\cos(\alpha\pm\beta),\,\tan(\alpha\pm\beta)$, 배각·반각·합곱 변환)
- ❌ 삼각함수의 극한·미분 ($\lim_{x\to 0}\sin x/x$, $(\sin x)'$ 등)
- ❌ 합성함수 미분, 매개변수 미분, 음함수 미분
- ❌ 부분적분, 치환적분, 정적분의 응용 (회전체 부피 등 미적분Ⅱ 영역)
- ❌ 수열의 극한·급수, 함수의 극한 일부 (확장된 부분)

**활용 가능 (수학Ⅰ 범위)**:
- ✅ 삼각함수의 정의·그래프·주기, 사인법칙·코사인법칙
- ✅ 일반각의 삼각함수, 단위원 기반 사고
- ✅ 지수·로그 함수, 수열의 합 (시그마)

**활용 가능 (수학Ⅱ 범위)**:
- ✅ 다항함수의 극한·미분·적분, 도함수 활용 (극값·접선·증감)
- ✅ 정적분의 기본정리, 다항함수 정적분의 도형 응용

문항이나 풀이에서 위 ❌ 영역을 사용해야 한다면, **다른 풀이로 우회**하거나 (특수각·기하적 풀이 등) 해당 문항을 빼고 다른 패턴으로 대체.
