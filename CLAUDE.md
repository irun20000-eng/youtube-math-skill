...
**자동화 체인 (2026-05-10 구축, 2026-05-11 보강)**:
...
**🚨 통합 워크플로우 실패 시 fallback (2026-05-11 검증된 우회)**:
auto-merge 워크플로우가 main 머지 후 Pages 배포 단계에서 실패하거나 큐에 갇혀 갤러리가 갱신되지 않을 경우:
1. `mcp__github__create_or_update_file` 로 main 의 `output/INDEX.md` 타임스탬프만 살짝 갱신해 push
2. 이 push 는 사용자 본인 계정(`irun20`)으로 처리되어 일반 push event 로 인식 → `deploy-pages.yml` 정상 트리거
